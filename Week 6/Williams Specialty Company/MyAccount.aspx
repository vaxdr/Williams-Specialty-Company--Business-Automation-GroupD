...
<body>
    ...
    &nbsp;&nbsp;&nbsp;
        ...
        <br />
        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString4 %>" DeleteCommand="DELETE FROM [Address] WHERE [AddressID] = ?" InsertCommand="INSERT INTO [Address] ([AddressID], [AddressLine1], [AddressLine2], [City], [StateCode], [PostalCode]) VALUES (?, ?, ?, ?, ?, ?)" ProviderName="<%$ ConnectionStrings:ConnectionString4.ProviderName %>" SelectCommand="SELECT * FROM [Address] WHERE [AddressID] = ?" UpdateCommand="UPDATE [Address] SET [AddressLine1] = ?, [AddressLine2] = ?, [City] = ?, [StateCode] = ?, [PostalCode] = ? WHERE [AddressID] = ?">
            <DeleteParameters>
                <asp:Parameter Name="AddressID" Type="Int32" />
            </DeleteParameters>
            <InsertParameters>
        ...
        <br />
        <br />
        <asp:GridView ID="grdOrders" runat="server" CellPadding="4" ForeColor="#333333" GridLines="None" AutoGenerateColumns="False" DataKeyNames="OrderID,OrderDetailID" DataSourceID="SqlDataSource3">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
                ...
                <asp:BoundField DataField="ProdTotal" DataFormatString="{0:c}" HeaderText="ProdTotal" SortExpression="ProdTotal" />
        ...
        <br />
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString4 %>" DeleteCommand="DELETE FROM [PurchaseOrderDetail] WHERE [OrderID] = ? AND [OrderDetailID] = ?" InsertCommand="INSERT INTO [PurchaseOrderDetail] ([OrderID], [OrderDetailID], [OrderQty], [ProdID], [Message], [ProdTotal]) VALUES (?, ?, ?, ?, ?, ?)" ProviderName="<%$ ConnectionStrings:ConnectionString4.ProviderName %>" SelectCommand="SELECT OrderID, OrderDetailID, OrderQty, ProdID, Message, ProdTotal, CustID FROM PurchaseOrderDetail WHERE (CustID = ?)" UpdateCommand="UPDATE [PurchaseOrderDetail] SET [OrderQty] = ?, [ProdID] = ?, [Message] = ?, [ProdTotal] = ? WHERE [OrderID] = ? AND [OrderDetailID] = ?">
            <DeleteParameters>
