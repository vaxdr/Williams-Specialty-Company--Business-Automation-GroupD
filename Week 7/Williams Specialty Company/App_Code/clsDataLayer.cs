...
11/6/2019 Added Login and Logout buttons.  Added Log In web form.  Login button visible by defaut Log out button not visible by default.  Program will handle this when loaded When you click the log in button, it should take you to either the My Account page if secLvl is low, else it takes you to Order info page - Daniel Crowder
...
11/21/2019 added if statement to hide grdUsers, drop down security level, and security label unless user security level == "O" so customers can’t edits users - Joey Muzzo
